...
document.getElementById('second2third').addEventListener('click', async ()=>{
    ...
    try {
        ...
        document.getElementById('third_step_status').innerText = "請開啟手機NFC功能，並將卡片放置於NFC感應區";
    
        ndef.addEventListener("readingerror", () => {
            document.getElementById('third_step_status').innerText = "讀取時發生錯誤，請移開卡片，然後再試一次";
        });
    
        ndef.addEventListener("reading", ({ message, serialNumber }) => {
            document.getElementById('third_step_status').innerHTML = `已成功讀取卡片序號：${serialNumber}<br/>請按送出以完成卡片註冊`;
        });
      } catch (error) {
            document.getElementById('third_step_status').innerText = "初始化 NDEFReader 元件發生無法復原之錯誤，可能是您的瀏覽器或裝置並不支援WebNFC API，請使用其他瀏覽器或裝置並再試一次";
